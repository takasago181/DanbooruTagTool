...
SIDECAR=Path('docs/issue118/research_sidecar_v70.csv')
OUT=Path('docs/issue118/remaining_small_clusters_review_v104')
BATCH_ROWS=100
EXPECTED_ELIGIBLE=143
...
def main():
    ...
    rows=[{'identity_key':key,'candidate_cluster':ck,'candidate_state':'FULL_REVIEW_REMAINING_SMALL_CLUSTERS_V104_BATCH2','human_intent':'','review_note':''} for ck,key in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v104.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary={'issue':118,'mode':'REMAINING_SMALL_CLUSTERS_FULL_REVIEW_V104_BATCH2','source_sidecar':'research_sidecar_v70.csv',
    ...
    (OUT/'summary_v104.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
