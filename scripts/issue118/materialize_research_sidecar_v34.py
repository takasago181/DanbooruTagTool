...
BASE = Path('docs/issue118/research_sidecar_v33.csv')
REVIEW = Path('docs/issue118/mixed_five_cluster_review_v59/review_policy_v59.json')
CANDIDATE = Path('docs/issue118/mixed_five_cluster_review_v59/full_review_candidate_v59.csv')
REVIEW_BLOB = 'b5439f73f852405ba22a94c30c3164d66714f581'
CANDIDATE_BLOB = 'b4f49e5939fed8eb03d077239dc07569fc7c7a95'
OUT = Path('docs/issue118/research_sidecar_v34.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v34.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V59 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V59 fixed candidate blob SHA drift')
    ...
    if len(source) != 91 or review.get('source_rows') != 91:
        raise SystemExit('V59 source count drift')
    if review.get('review_verdicts_generated_by_materializer') != 'NO':
        raise SystemExit('V59 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs') != 'YES':
        raise SystemExit('V59 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V59 has unexpected unclassified rows')
    ...
    for intent, keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status'] != 'UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V59 not cleanly unclassified: {key}')
            target['sexual_intent'] = intent
            target['review_status'] = 'HUMAN_REVIEWED'
            target['rule_id'] = f'HUMAN_{intent}_V59'
            target['evidence'] = 'full semantic review of fixed V59 candidate set'
            promoted[intent] += 1
    ...
    expected_classes = {'CONTEXTUAL': 1073, 'NON_SEXUAL': 23559, 'NULL': 5508, 'SEXUAL': 1612}
    ...
    summary = {
        'issue': 118,
        'mode': 'RESEARCH_SIDECAR_V34_V59_FULL_REVIEW',
