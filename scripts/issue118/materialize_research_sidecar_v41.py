...
BASE = Path('docs/issue118/research_sidecar_v40.csv')
REVIEW = Path('docs/issue118/general_body_part_none_review_v66/review_policy_v66.json')
CANDIDATE = Path('docs/issue118/general_body_part_none_review_v66/full_review_candidate_v66.csv')
REVIEW_BLOB = '7bbd98eb3835b50b3a6025934ff8c4676f6378a1'
CANDIDATE_BLOB = '963ea13475b80a3ba0599fe14a2449e20227c883'
OUT = Path('docs/issue118/research_sidecar_v41.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v41.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V66 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V66 fixed candidate blob SHA drift')
    ...
    if len(source)!=100 or review.get('source_rows')!=100:
        raise SystemExit('V66 source count drift')
    if review.get('review_verdicts_generated_by_materializer')!='NO':
        raise SystemExit('V66 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs')!='YES':
        raise SystemExit('V66 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V66 has unexpected unclassified rows')
    ...
    for intent,keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status']!='UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V66 not cleanly unclassified: {key}')
            target['sexual_intent']=intent
            target['review_status']='HUMAN_REVIEWED'
            target['rule_id']=f'HUMAN_{intent}_V66'
            target['evidence']='full semantic review of fixed V66 candidate set'
            promoted[intent]+=1
    ...
    expected_status={'AUTO_HIGH_CONF':22371,'HUMAN_REVIEWED':4416,'UNCLASSIFIED':4965}
    expected_classes={'CONTEXTUAL':1212,'NON_SEXUAL':23874,'NULL':4965,'SEXUAL':1701}
    ...
    summary={
        'issue':118,
        'mode':'RESEARCH_SIDECAR_V41_V66_FULL_REVIEW',
