...
SIDECAR = Path('docs/issue118/research_sidecar_v30.csv')
OUT = Path('docs/issue118/overlap_action_three_cluster_review_v55')

TARGETS = {
    ('ACTION_CONTACT','FLUID_STATE_ACTION','NONE'): ('OVERLAP|ACTION_CONTACT|FLUID_STATE_ACTION|NONE', 27),
    ('ACTION_CONTACT','DIRECT_COMPOSITE','ANATOMY'): ('OVERLAP|ACTION_CONTACT|DIRECT_COMPOSITE|ANATOMY', 26),
    ('ACTION_CONTACT','(none)','NONE'): ('OVERLAP|ACTION_CONTACT|(none)|NONE', 25),
}
...
def main() -> int:
    ...
    for r in side:
        ...
        rows.append({
            ...
            'candidate_state': 'FULL_REVIEW_OVERLAP_ACTION_THREE_CLUSTER_V55',
            'human_intent': '',
            'review_note': '',
        })
    ...
    if len(rows) != 78:
        raise SystemExit(f'expected 78 rows, got {len(rows)}')
    ...
    with (OUT / 'full_review_candidate_v55.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'OVERLAP_ACTION_THREE_CLUSTER_FULL_REVIEW_V55',
        'source_sidecar': 'research_sidecar_v30.csv',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 78 rows',
    }
    (OUT / 'summary_v55.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
