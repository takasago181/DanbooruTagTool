...
SIDECAR = Path('docs/issue118/research_sidecar_v54.csv')
OUT = Path('docs/issue118/general_action_interaction_none_review_v81')
EXPECTED_CLUSTER_ROWS = 195
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows=[{
        ...
        'candidate_state':'FULL_REVIEW_GENERAL_ACTION_INTERACTION_NONE_V81_BATCH7',
        'human_intent':'',
        'review_note':'',
    } for key in chosen]
    ...
    with (OUT/'full_review_candidate_v81.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n')
        w.writeheader(); w.writerows(rows)

    summary={
        'issue':118,
        'mode':'GENERAL_ACTION_INTERACTION_NONE_FULL_REVIEW_V81_BATCH7',
        'source_sidecar':'research_sidecar_v54.csv',
        'source_cluster':'GENERAL_ONLY|ACTION_CONTACT/INTERACTION|NONE',
        'source_cluster_rows':len(cluster),
        'candidate_rows':len(rows),
        'batch_selection':'alphabetically first 100 identities from the remaining fixed General-only Action/Interaction cluster after V80; selection only, not classification authority',
    ...
    (OUT/'summary_v81.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
