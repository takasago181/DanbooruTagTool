...
SIDECAR = Path('docs/issue118/research_sidecar_v36.csv')
OUT = Path('docs/issue118/mixed_seven_cluster_review_v61')

TARGETS = {
    'OVERLAP|PERSON_COUNT|RELATION_ROLE_CONTEXT|NONE': 16,
    'OVERLAP|ACTION_CONTACT|INSERTION_STRUCTURED|NONE': 15,
    'GENERAL_ONLY|PERSON_COUNT|NONE': 14,
    'OVERLAP|OBJECT_PROP|RESTRAINT_IMPLEMENT|NONE': 13,
    'OVERLAP|OBJECT_PROP|RESTRAINT_IMPLEMENT|RESTRAINT_FETISH': 13,
    'OVERLAP|BODY_PART|FLUID_STATE_ACTION|ANATOMY': 12,
    'GENERAL_ONLY|CLOTHING/EVERYDAY|ADULT_ROLE_DEVICE': 12,
}
...
def main() -> int:
    ...
    for r in side:
        ...
        rows.append({
            ...
            'candidate_state': 'FULL_REVIEW_MIXED_SEVEN_CLUSTER_V61',
            'human_intent': '',
            'review_note': '',
        })
    ...
    with (OUT / 'full_review_candidate_v61.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'MIXED_SEVEN_CLUSTER_FULL_REVIEW_V61',
        'source_sidecar': 'research_sidecar_v36.csv',
    ...
    (OUT / 'summary_v61.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
