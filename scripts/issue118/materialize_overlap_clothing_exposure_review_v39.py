...
SIDECAR=Path('docs/issue118/research_sidecar_v23.csv')
OUT=Path('docs/issue118/overlap_clothing_exposure_review_v39')
...
def main():
    ...
    for r in side:
        ...
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_OVERLAP_CLOTHING_EXPOSURE_V39','human_intent':'','review_note':''})
    rows.sort(key=lambda r:r['identity_key'])
    if len(rows)!=83:raise SystemExit(f'expected 83 rows, got {len(rows)}')
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'full_review_candidate_v39.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    s={'issue':118,'mode':'OVERLAP_CLOTHING_EXPOSURE_FULL_REVIEW_V39','candidate_rows':83,'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 83 rows'}
    (OUT/'summary_v39.json').write_text(json.dumps(s,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(s,ensure_ascii=False,sort_keys=True));return 0
