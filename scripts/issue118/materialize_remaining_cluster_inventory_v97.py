...
OUT = Path('docs/issue118/remaining_cluster_inventory_v97')
EXPECTED_REMAINING = 2681
...
def main() -> int:
    ...
    with (OUT / 'cluster_inventory_v97.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V97_FROM_SIDECAR_V67',
    ...
    (OUT / 'summary_v97.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
