...
BASE = Path('docs/issue118/research_sidecar_v35.csv')
REVIEW = Path('docs/issue118/exposure_six_cluster_review_v60/review_policy_v60.json')
CANDIDATE = Path('docs/issue118/exposure_six_cluster_review_v60/full_review_candidate_v60.csv')
REVIEW_BLOB = '0312a44c7d589dde367c3efe4526624d4d2816b8'
CANDIDATE_BLOB = '2cbd10c8c704d572d0f53cb1062151d533dde0a4'
OUT = Path('docs/issue118/research_sidecar_v36.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v36.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V60 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V60 fixed candidate blob SHA drift')
    ...
    if len(source) != 89 or review.get('source_rows') != 89:
        raise SystemExit('V60 source count drift')
    if review.get('review_verdicts_generated_by_materializer') != 'NO':
        raise SystemExit('V60 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs') != 'YES':
        raise SystemExit('V60 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V60 has unexpected unclassified rows')
    ...
    for intent, keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status'] != 'UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V60 not cleanly unclassified: {key}')
            target['sexual_intent'] = intent
            target['review_status'] = 'HUMAN_REVIEWED'
            target['rule_id'] = f'HUMAN_{intent}_V60'
            target['evidence'] = 'full semantic review of fixed V60 candidate set'
            promoted[intent] += 1
    ...
    expected_status = {'AUTO_HIGH_CONF': 22371, 'HUMAN_REVIEWED': 3962, 'UNCLASSIFIED': 5419}
    expected_classes = {'CONTEXTUAL': 1119, 'NON_SEXUAL': 23567, 'NULL': 5419, 'SEXUAL': 1647}
    ...
    summary = {
        'issue': 118,
        'mode': 'RESEARCH_SIDECAR_V36_V60_FULL_REVIEW',
