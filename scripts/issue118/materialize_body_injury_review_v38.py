...
OUT=Path('docs/issue118/body_injury_review_v38')
LEX_GROUPS={
...
def main():
    ...
    for r in side:
        ...
        if gr is None or (gr.get('primary_path') or '').strip()!='BODY_PART':continue
        if flags(key)!='INJURY':continue
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_BODY_INJURY_V38','human_intent':'','review_note':''})
    rows.sort(key=lambda r:r['identity_key'])
    if len(rows)!=58:raise SystemExit(f'expected 58 rows, got {len(rows)}')
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'full_review_candidate_v38.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    s={'issue':118,'mode':'BODY_INJURY_FULL_REVIEW_V38','candidate_rows':58,'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 58 rows'}
    (OUT/'summary_v38.json').write_text(json.dumps(s,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(s,ensure_ascii=False,sort_keys=True));return 0
