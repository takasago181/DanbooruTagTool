...
SIDECAR = Path('docs/issue118/research_sidecar_v39.csv')
OUT = Path('docs/issue118/general_clothing_everyday_none_review_v65')
EXPECTED_CLUSTER_ROWS = 60
BATCH_ROWS = 60
...
def main() -> int:
    ...
    rows = [{
        ...
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_EVERYDAY_NONE_V65_BATCH3',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v65.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_EVERYDAY_NONE_FULL_REVIEW_V65_BATCH3',
        'source_sidecar': 'research_sidecar_v39.csv',
        'source_cluster': 'GENERAL_ONLY|CLOTHING/EVERYDAY|NONE',
        'source_cluster_rows': len(cluster),
        'candidate_rows': len(rows),
        'batch_selection': 'all 60 remaining identities from the fixed source cluster after V64; selection only, not classification authority',
    ...
    (OUT / 'summary_v65.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
