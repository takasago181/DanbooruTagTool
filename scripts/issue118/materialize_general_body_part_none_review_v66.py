...
SIDECAR = Path('docs/issue118/research_sidecar_v40.csv')
OUT = Path('docs/issue118/general_body_part_none_review_v66')
EXPECTED_CLUSTER_ROWS = 241
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows=[{
        ...
        'candidate_state':'FULL_REVIEW_GENERAL_BODY_PART_NONE_V66_BATCH1',
        'human_intent':'',
        'review_note':'',
    } for key in chosen]
    ...
    with (OUT/'full_review_candidate_v66.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n')
        w.writeheader(); w.writerows(rows)

    summary={
        'issue':118,
        'mode':'GENERAL_BODY_PART_NONE_FULL_REVIEW_V66_BATCH1',
        'source_sidecar':'research_sidecar_v40.csv',
        'source_cluster':'GENERAL_ONLY|BODY_PART|NONE',
        'source_cluster_rows':len(cluster),
        'candidate_rows':len(rows),
        'batch_selection':'alphabetically first 100 identities from fixed source cluster; selection only, not classification authority',
    ...
    (OUT/'summary_v66.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
