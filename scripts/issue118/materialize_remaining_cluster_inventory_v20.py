...
OUT = Path('docs/issue118/remaining_cluster_inventory_v20')
...
def main() -> int:
    ...
    if len(rows) != 8215:
        raise SystemExit(f'expected 8215 remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    with (OUT / 'cluster_inventory_v20.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=fields, lineterminator='\n')
        w.writeheader(); w.writerows(inventory)

    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V20_FROM_SIDECAR_V8',
        'source_sidecar': 'research_sidecar_v8.csv',
    ...
    (OUT / 'summary_v20.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
