...
SIDECAR = Path('docs/issue118/research_sidecar_v47.csv')
OUT = Path('docs/issue118/general_clothing_everyday_sexualized_review_v73')
EXPECTED_CLUSTER_ROWS = 25
BATCH_ROWS = 25
...
def main() -> int:
    ...
    rows = [{
        ...
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_EVERYDAY_SEXUALIZED_V73_BATCH5',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v73.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_EVERYDAY_SEXUALIZED_FULL_REVIEW_V73_BATCH5',
        'source_sidecar': 'research_sidecar_v47.csv',
        'source_cluster': 'GENERAL_ONLY|CLOTHING/EVERYDAY|SEXUALIZED_CLOTHING',
        'source_cluster_rows': len(cluster),
        'candidate_rows': len(rows),
        'batch_selection': 'all 25 remaining identities from the fixed sexualized-clothing discovery cluster after V72; selection only, not classification authority',
    ...
    (OUT / 'summary_v73.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
