...
OUT = Path('docs/issue118/action_five_cluster_review_v58')

TARGETS = {
    ('ACTION_CONTACT','(none)','ANATOMY'): ('OVERLAP|ACTION_CONTACT|(none)|ANATOMY', 23),
    ('ACTION_CONTACT','NONHUMAN_INTERACTION','NONE'): ('OVERLAP|ACTION_CONTACT|NONHUMAN_INTERACTION|NONE', 20),
    ('ACTION_CONTACT','ACTION_INTERACTION','EXPLICIT_SEX'): ('OVERLAP|ACTION_CONTACT|ACTION_INTERACTION|EXPLICIT_SEX', 18),
    ('ACTION_CONTACT','(none)','EXPLICIT_SEX'): ('OVERLAP|ACTION_CONTACT|(none)|EXPLICIT_SEX', 17),
    ('ACTION_CONTACT','CONTEXT_MODIFIER','EXPLICIT_SEX'): ('OVERLAP|ACTION_CONTACT|CONTEXT_MODIFIER|EXPLICIT_SEX', 17),
}
...
def main() -> int:
    ...
    for r in side:
        ...
        rows.append({
            ...
            'candidate_state': 'FULL_REVIEW_ACTION_FIVE_CLUSTER_V58',
            'human_intent': '',
            'review_note': '',
        })
    ...
    if len(rows) != 95:
        raise SystemExit(f'expected 95 rows, got {len(rows)}')
    ...
    with (OUT / 'full_review_candidate_v58.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'ACTION_FIVE_CLUSTER_FULL_REVIEW_V58',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 95 rows',
    }
    (OUT / 'summary_v58.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
