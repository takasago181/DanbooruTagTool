...
BASE = Path('docs/issue118/research_sidecar_v37.csv')
REVIEW = Path('docs/issue118/general_clothing_everyday_none_review_v63/review_policy_v63.json')
CANDIDATE = Path('docs/issue118/general_clothing_everyday_none_review_v63/full_review_candidate_v63.csv')
REVIEW_BLOB = '26876c4bf4b011a4bf812c51ec44096cd52648b2'
CANDIDATE_BLOB = '30afc8083b0f98d6b0c9ef18e877166f190d3871'
OUT = Path('docs/issue118/research_sidecar_v38.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v38.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V63 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V63 fixed candidate blob SHA drift')
    ...
    if len(source) != 100 or review.get('source_rows') != 100:
        raise SystemExit('V63 source count drift')
    if review.get('review_verdicts_generated_by_materializer') != 'NO':
        raise SystemExit('V63 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs') != 'YES':
        raise SystemExit('V63 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V63 has unexpected unclassified rows')
    ...
    for intent, keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status'] != 'UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V63 not cleanly unclassified: {key}')
            target['sexual_intent'] = intent
            target['review_status'] = 'HUMAN_REVIEWED'
            target['rule_id'] = f'HUMAN_{intent}_V63'
            target['evidence'] = 'full semantic review of fixed V63 candidate set'
            promoted[intent] += 1
    ...
    expected_status = {'AUTO_HIGH_CONF': 22371, 'HUMAN_REVIEWED': 4156, 'UNCLASSIFIED': 5225}
    expected_classes = {'CONTEXTUAL': 1169, 'NON_SEXUAL': 23660, 'NULL': 5225, 'SEXUAL': 1698}
    ...
    summary = {
        'issue': 118,
        'mode': 'RESEARCH_SIDECAR_V38_V63_FULL_REVIEW',
