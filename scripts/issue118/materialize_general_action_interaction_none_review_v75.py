...
OUT = Path('docs/issue118/general_action_interaction_none_review_v75')
EXPECTED_CLUSTER_ROWS = 795
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows=[{
        ...
        'candidate_state':'FULL_REVIEW_GENERAL_ACTION_INTERACTION_NONE_V75_BATCH1',
        'human_intent':'',
        'review_note':'',
    } for key in chosen]
    ...
    with (OUT/'full_review_candidate_v75.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n')
        w.writeheader(); w.writerows(rows)

    summary={
        'issue':118,
        'mode':'GENERAL_ACTION_INTERACTION_NONE_FULL_REVIEW_V75_BATCH1',
        ...
        'batch_selection':'alphabetically first 100 identities from fixed General-only Action/Interaction cluster; selection only, not classification authority',
    ...
    (OUT/'summary_v75.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
