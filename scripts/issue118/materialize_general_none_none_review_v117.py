...
SIDECAR=Path('docs/issue118/research_sidecar_v83.csv')
OUT=Path('docs/issue118/general_none_none_review_v117')
EXPECTED_CLUSTER_ROWS=1139
EXPECTED_ELIGIBLE_ROWS=1137
BATCH_ROWS=100
...
def main():
    ...
    if len(eligible)!=EXPECTED_ELIGIBLE_ROWS:
        raise SystemExit(f'expected {EXPECTED_ELIGIBLE_ROWS} eligible giant-cluster rows, got {len(eligible)}')
    chosen=eligible[:BATCH_ROWS]
    rows=[{
      'identity_key':key,
      'candidate_cluster':'GENERAL_ONLY|(none)|NONE',
      'candidate_state':'FULL_REVIEW_GENERAL_NONE_NONE_V117_BATCH12',
      'human_intent':'',
      'review_note':''
    } for key in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v117.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary={
      'issue':118,'mode':'GENERAL_NONE_NONE_FULL_REVIEW_V117_BATCH12',
      'source_sidecar':'research_sidecar_v83.csv',
      'source_cluster':'GENERAL_ONLY|(none)|NONE',
      'source_cluster_rows':len(pool),
      'retained_exclusions':sorted(RETAINED_EXCLUSIONS),
      'eligible_cluster_rows':len(eligible),
      'candidate_rows':len(rows),
      'selection':'alphabetically first 100 identities from remaining fixed giant General-only no-path no-lex cluster after V116, excluding previously retained unresolved identities; selection only, not classification authority',
    ...
    (OUT/'summary_v117.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
