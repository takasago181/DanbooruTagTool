...
SUMMARY=Path('docs/issue118/research_sidecar_summary_v79.json')
...
def main():
    if git_blob_sha(REVIEW)!=REVIEW_BLOB: raise SystemExit('V112 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE)!=CANDIDATE_BLOB: raise SystemExit('V112 fixed candidate blob SHA drift')
    base=read_csv(BASE); source=read_csv(CANDIDATE); review=json.loads(REVIEW.read_text(encoding='utf-8'))
    if len(base)!=31752 or len(source)!=100 or review.get('source_rows')!=100: raise SystemExit('V112 source/base drift')
    if review.get('review_verdicts_generated_by_materializer')!='NO' or review.get('review_artifacts_are_external_inputs')!='YES': raise SystemExit('V112 provenance drift')
    ...
    for intent,keys in groups.items():
        if not keys<=source_set or union & keys: raise SystemExit(f'V112 group drift {intent}')
        union|=keys
    if union!=source_set or review.get('unclassified'): raise SystemExit('V112 coverage drift')
    if {k:len(v) for k,v in groups.items()}!=review['decision_counts']: raise SystemExit('V112 decision count drift')
    index={r['identity_key']:dict(r) for r in base}; promoted=Counter()
    for intent,keys in groups.items():
        for key in sorted(keys):
            t=index.get(key)
            if t is None or t['review_status']!='UNCLASSIFIED' or t['sexual_intent']: raise SystemExit(f'V112 target drift {key}')
            t['sexual_intent']=intent; t['review_status']='HUMAN_REVIEWED'; t['rule_id']=f'HUMAN_{intent}_V112'; t['evidence']='full semantic review of fixed V112 candidate set'; promoted[intent]+=1
    ...
    expected_status={'AUTO_HIGH_CONF':22371,'HUMAN_REVIEWED':7840,'UNCLASSIFIED':1541}
    expected_classes={'CONTEXTUAL':1923,'NON_SEXUAL':26275,'NULL':1541,'SEXUAL':2013}
    ...
    summary={'issue':118,'mode':'RESEARCH_SIDECAR_V78_V112_FULL_REVIEW','identity_rows':len(rows),'source_base':str(BASE),
      ...
      'review_inputs':[{'label':'V112','review_source':str(REVIEW),'review_git_blob_sha':REVIEW_BLOB,'review_sha256':sha256(REVIEW),'candidate_source':str(CANDIDATE),'candidate_git_blob_sha':CANDIDATE_BLOB,'source_rows':100}],
