...
OUT = Path('docs/issue118/remaining_cluster_inventory_v7')
...
def main() -> int:
    ...
    if len(rows) != 15880:
        raise SystemExit(f'expected 15880 remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    with (OUT / 'cluster_inventory_v7.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V7',
    ...
    (OUT / 'summary_v7.json').write_text(
