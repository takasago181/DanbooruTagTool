...
OUT = Path('docs/issue118/general_three_cluster_review_v54')

TARGETS = {
    'CLOTHING/COSTUME': ('GENERAL_ONLY|CLOTHING/COSTUME|NONE', 27),
    'ACTION_CONTACT': ('GENERAL_ONLY|ACTION_CONTACT|NONE', 25),
    'POSE_MOVEMENT': ('GENERAL_ONLY|POSE_MOVEMENT|NONE', 20),
}
...
def main() -> int:
    ...
    for r in side:
        ...
        if path not in TARGETS or flags(key) != 'NONE':
            continue
        cluster_key, _ = TARGETS[path]
        counts[cluster_key] += 1
        rows.append({
            'identity_key': key,
            'candidate_cluster': cluster_key,
            'candidate_state': 'FULL_REVIEW_GENERAL_THREE_CLUSTER_V54',
            'human_intent': '',
            'review_note': '',
        })
    ...
    if len(rows) != 72:
        raise SystemExit(f'expected 72 rows, got {len(rows)}')
    ...
    with (OUT / 'full_review_candidate_v54.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_THREE_CLUSTER_FULL_REVIEW_V54',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 72 rows',
    }
    (OUT / 'summary_v54.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
