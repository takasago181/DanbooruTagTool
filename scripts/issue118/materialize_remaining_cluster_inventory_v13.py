...
OUT = Path('docs/issue118/remaining_cluster_inventory_v13')
...
LEX_GROUPS = {
    ...
    'EXPOSURE_INTIMATE': {
        'nude','naked','topless','bottomless','panty','panties','underwear','bra','cleavage','underboob',
    ...
    'RELATIONSHIP_ROLE': {
        'brocon','siscon','lolicon','shotacon','incest','virgin','virginity','seme','uke','femdom',
        'maledom','ageplay','cuckold','cuckquean','netorare','netori','ntr',
    },
    'SEXUALIZED_CLOTHING': {
        'bikini','swimsuit','swimwear','thong','garter','garters','gstring','fishnet','fishnets',
        'corset','harness','latex','leotard','bodysuit','bunnysuit','playboy','fetishwear','bodycon',
        'slingshot','highleg','lowleg','sheer','transparent','frontless','backless','assless','sideless',
        'tankini','monokini','trikini','microdress','dongtan',
    },
}
...
def flags(key: str) -> str:
    ...
    clothing_patterns = [
        ...
        r'(?:^|[_/\-])see[_\-]through(?:$|[_/\-])',
        r'(?:^|[_/\-])t[_\-]back(?:$|[_/\-])',
        r'(?:^|[_/\-])micro(?:dress|skirt|shorts|pants|top|shirt)(?:$|[_/\-])',
    ]
    if any(re.search(p, k) for p in clothing_patterns):
        hit.append('SEXUALIZED_CLOTHING')
    if re.search(r'(?:^|[_/\-])top[/_-]bottom[_/-]dynamic(?:$|[_/\-])', k):
        hit.append('RELATIONSHIP_ROLE')
    if re.search(r'(?:^|[_/\-])bottom[/_-]top[_/-]dynamic(?:$|[_/\-])', k):
        hit.append('RELATIONSHIP_ROLE')
    hit = sorted(set(hit))
...
def main() -> int:
    ...
    for r in side:
        ...
        gr = g.get(key)
        sr = s.get(key)
        if gr is not None and r['is_special'] == 'YES':
            membership = 'OVERLAP'
        elif gr is not None:
    ...
    if len(rows) != 11434:
        raise SystemExit(f'expected 11434 remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    for ck, items in clusters.items():
        ...
        inventory.append({
            ...
            'general_root': a['general_root'],
            'generation_family': a['generation_family'],
            'lex_flags': a['lex_flags'],
            'examples': ';'.join(x['identity_key'] for x in sorted(items, key=lambda x: x['identity_key'])[:12]),
        })
    ...
    with (OUT / 'cluster_inventory_v13.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V13_FROM_SIDECAR_V5',
        'source_sidecar': 'research_sidecar_v5.csv',
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
        'clusters_ge_20': sum(1 for x in inventory if int(x['rows']) >= 20),
        'clusters_ge_100': sum(1 for x in inventory if int(x['rows']) >= 100),
        'top10_cluster_rows': sum(int(x['rows']) for x in inventory[:10]),
        'top20_cluster_rows': sum(int(x['rows']) for x in inventory[:20]),
        'top30_cluster_rows': sum(int(x['rows']) for x in inventory[:30]),
        'top_general_roots': root_counts.most_common(20),
        'top_general_paths': path_counts.most_common(30),
        'top_generation_families': family_counts.most_common(20),
        'lex_flag_counts': dict(sorted(lex_counts.items())),
        'top_clusters': inventory[:40],
        'lexical_flags_are_discovery_only': 'YES',
    ...
    (OUT / 'summary_v13.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
        'top20_cluster_rows': summary['top20_cluster_rows'],
