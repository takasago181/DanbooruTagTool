...
SIDECAR = Path('docs/issue118/research_sidecar_v57.csv')
OUT = Path('docs/issue118/remaining_cluster_inventory_v83')
EXPECTED_REMAINING = 3606
...
def main() -> int:
    ...
    if len(rows) != EXPECTED_REMAINING:
        raise SystemExit(f'expected {EXPECTED_REMAINING} remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    with (OUT / 'cluster_inventory_v83.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V83_FROM_SIDECAR_V57',
        'source_sidecar': 'research_sidecar_v57.csv',
    ...
    (OUT / 'summary_v83.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
