...
BASE = Path('docs/issue118/research_sidecar_v41.csv')
REVIEW = Path('docs/issue118/general_body_part_none_review_v67/review_policy_v67.json')
CANDIDATE = Path('docs/issue118/general_body_part_none_review_v67/full_review_candidate_v67.csv')
REVIEW_BLOB = 'c06329fe7cb567e1b3e918044d7548d204ffe029'
CANDIDATE_BLOB = '8f1d7315c27de6afc7d8a2a2923279495609f8e7'
OUT = Path('docs/issue118/research_sidecar_v42.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v42.json')
...
def main() -> int:
    if git_blob_sha(REVIEW)!=REVIEW_BLOB:
        raise SystemExit('V67 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE)!=CANDIDATE_BLOB:
        raise SystemExit('V67 fixed candidate blob SHA drift')
    base=read_csv(BASE)
    ...
    if len(source)!=100 or review.get('source_rows')!=100:
        raise SystemExit('V67 source count drift')
    if review.get('review_verdicts_generated_by_materializer')!='NO':
        raise SystemExit('V67 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs')!='YES':
        raise SystemExit('V67 provenance drift: external input flag')
    ...
    if unclassified!={'knee_boobs','powerful_ass'}:
        raise SystemExit(f'unexpected V67 unclassified set {sorted(unclassified)}')
    ...
    for intent,keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status']!='UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V67 not cleanly unclassified: {key}')
            target['sexual_intent']=intent
            target['review_status']='HUMAN_REVIEWED'
            target['rule_id']=f'HUMAN_{intent}_V67'
            target['evidence']='full semantic review of fixed V67 candidate set'
            promoted[intent]+=1
    for key in unclassified:
        target=index.get(key)
        if target is None or target['review_status']!='UNCLASSIFIED' or target['sexual_intent']:
            raise SystemExit(f'V67 retained unclassified row drift: {key}')
    ...
    expected_status={'AUTO_HIGH_CONF':22371,'HUMAN_REVIEWED':4514,'UNCLASSIFIED':4867}
    expected_classes={'CONTEXTUAL':1222,'NON_SEXUAL':23962,'NULL':4867,'SEXUAL':1701}
    ...
    summary={
        'issue':118,
        'mode':'RESEARCH_SIDECAR_V42_V67_FULL_REVIEW',
