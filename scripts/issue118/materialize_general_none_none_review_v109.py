...
SIDECAR=Path('docs/issue118/research_sidecar_v75.csv')
OUT=Path('docs/issue118/general_none_none_review_v109')
EXPECTED_CLUSTER_ROWS=1937
BATCH_ROWS=100
...
def main():
    ...
    rows=[{
      ...
      'candidate_state':'FULL_REVIEW_GENERAL_NONE_NONE_V109_BATCH4',
      'human_intent':'',
      'review_note':''
    } for key in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v109.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary={
      'issue':118,'mode':'GENERAL_NONE_NONE_FULL_REVIEW_V109_BATCH4',
      'source_sidecar':'research_sidecar_v75.csv',
      'source_cluster':'GENERAL_ONLY|(none)|NONE',
      'source_cluster_rows':len(pool),
      'candidate_rows':len(rows),
      'selection':'alphabetically first 100 identities from remaining fixed giant General-only no-path no-lex cluster after V108; selection only, not classification authority',
    ...
    (OUT/'summary_v109.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
