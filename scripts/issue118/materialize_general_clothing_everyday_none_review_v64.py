...
SIDECAR = Path('docs/issue118/research_sidecar_v38.csv')
OUT = Path('docs/issue118/general_clothing_everyday_none_review_v64')
EXPECTED_CLUSTER_ROWS = 160
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows = [{
        'identity_key': key,
        'candidate_cluster': 'GENERAL_ONLY|CLOTHING/EVERYDAY|NONE',
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_EVERYDAY_NONE_V64_BATCH2',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v64.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_EVERYDAY_NONE_FULL_REVIEW_V64_BATCH2',
        'source_sidecar': 'research_sidecar_v38.csv',
        'source_cluster': 'GENERAL_ONLY|CLOTHING/EVERYDAY|NONE',
        'source_cluster_rows': len(cluster),
        'candidate_rows': len(rows),
        'batch_selection': 'alphabetically first 100 identities from the remaining fixed source cluster after V63; selection only, not classification authority',
    ...
    (OUT / 'summary_v64.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
