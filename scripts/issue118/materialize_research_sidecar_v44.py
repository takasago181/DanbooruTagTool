...
BASE = Path('docs/issue118/research_sidecar_v43.csv')
REVIEW = Path('docs/issue118/general_clothing_everyday_sexualized_review_v69/review_policy_v69.json')
CANDIDATE = Path('docs/issue118/general_clothing_everyday_sexualized_review_v69/full_review_candidate_v69.csv')
REVIEW_BLOB = 'c6c17f8fe210eeae579afd4a6ef079772a543062'
CANDIDATE_BLOB = '93ddcc867dc586e16c8e1b445250fddd86c7b0bc'
OUT = Path('docs/issue118/research_sidecar_v44.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v44.json')
...
def main()->int:
    if git_blob_sha(REVIEW)!=REVIEW_BLOB:
        raise SystemExit('V69 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE)!=CANDIDATE_BLOB:
        raise SystemExit('V69 fixed candidate blob SHA drift')
    base=read_csv(BASE)
    ...
    if len(source)!=100 or review.get('source_rows')!=100:
        raise SystemExit('V69 source count drift')
    if review.get('review_verdicts_generated_by_materializer')!='NO':
        raise SystemExit('V69 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs')!='YES':
        raise SystemExit('V69 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V69 has unexpected unclassified rows')
    ...
    for intent,keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status']!='UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V69 not cleanly unclassified: {key}')
            target['sexual_intent']=intent
            target['review_status']='HUMAN_REVIEWED'
            target['rule_id']=f'HUMAN_{intent}_V69'
            target['evidence']='full semantic review of fixed V69 candidate set'
            promoted[intent]+=1
    ...
    expected_status={'AUTO_HIGH_CONF':22371,'HUMAN_REVIEWED':4655,'UNCLASSIFIED':4726}
    expected_classes={'CONTEXTUAL':1251,'NON_SEXUAL':24074,'NULL':4726,'SEXUAL':1701}
    ...
    summary={
        'issue':118,
        'mode':'RESEARCH_SIDECAR_V44_V69_FULL_REVIEW',
