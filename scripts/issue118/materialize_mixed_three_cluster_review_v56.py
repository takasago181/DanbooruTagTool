...
SIDECAR = Path('docs/issue118/research_sidecar_v31.csv')
OUT = Path('docs/issue118/mixed_three_cluster_review_v56')

TARGETS = {
    ('ACTION_CONTACT','CONTEXT_MODIFIER','NONE'): ('OVERLAP|ACTION_CONTACT|CONTEXT_MODIFIER|NONE', 22),
    ('ACTION_CONTACT','POSE_COMPOSITION','NONE'): ('OVERLAP|ACTION_CONTACT|POSE_COMPOSITION|NONE', 22),
    ('BODY_PART','DAMAGE_STATE_ACTION','INJURY'): ('OVERLAP|BODY_PART|DAMAGE_STATE_ACTION|INJURY', 22),
}
...
def main() -> int:
    ...
    for r in side:
        ...
        rows.append({
            ...
            'candidate_state': 'FULL_REVIEW_MIXED_THREE_CLUSTER_V56',
            'human_intent': '',
            'review_note': '',
        })
    ...
    with (OUT / 'full_review_candidate_v56.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'MIXED_THREE_CLUSTER_FULL_REVIEW_V56',
        'source_sidecar': 'research_sidecar_v31.csv',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 66 rows',
    }
    (OUT / 'summary_v56.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
