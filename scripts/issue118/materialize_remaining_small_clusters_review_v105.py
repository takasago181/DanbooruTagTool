...
SIDECAR=Path('docs/issue118/research_sidecar_v71.csv')
OUT=Path('docs/issue118/remaining_small_clusters_review_v105')
BATCH_ROWS=43
EXPECTED_ELIGIBLE=43
...
def main():
    ...
    rows=[{'identity_key':key,'candidate_cluster':ck,'candidate_state':'FULL_REVIEW_REMAINING_SMALL_CLUSTERS_V105_BATCH3','human_intent':'','review_note':''} for ck,key in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v105.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary={'issue':118,'mode':'REMAINING_SMALL_CLUSTERS_FULL_REVIEW_V105_BATCH3','source_sidecar':'research_sidecar_v71.csv',
      ...
      'batch_selection':'all 43 rows sorted by cluster_key then identity_key from all remaining non-giant clusters after excluding prior intentional UNCLASSIFIED rows; selection only, not classification authority',
    ...
    (OUT/'summary_v105.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
