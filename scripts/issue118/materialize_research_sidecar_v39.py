...
BASE = Path('docs/issue118/research_sidecar_v38.csv')
REVIEW = Path('docs/issue118/general_clothing_everyday_none_review_v64/review_policy_v64.json')
CANDIDATE = Path('docs/issue118/general_clothing_everyday_none_review_v64/full_review_candidate_v64.csv')
REVIEW_BLOB = '57eaddeb88e47208237bd312dab1056aa8a716f4'
CANDIDATE_BLOB = '50f6935e7371c3e661b95a34a53f13e78322023f'
OUT = Path('docs/issue118/research_sidecar_v39.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v39.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V64 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V64 fixed candidate blob SHA drift')
    ...
    if len(source) != 100 or review.get('source_rows') != 100:
        raise SystemExit('V64 source count drift')
    if review.get('review_verdicts_generated_by_materializer') != 'NO':
        raise SystemExit('V64 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs') != 'YES':
        raise SystemExit('V64 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V64 has unexpected unclassified rows')
    ...
    for intent, keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status'] != 'UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V64 not cleanly unclassified: {key}')
            target['sexual_intent'] = intent
            target['review_status'] = 'HUMAN_REVIEWED'
            target['rule_id'] = f'HUMAN_{intent}_V64'
            target['evidence'] = 'full semantic review of fixed V64 candidate set'
            promoted[intent] += 1
    ...
    expected_status = {'AUTO_HIGH_CONF': 22371, 'HUMAN_REVIEWED': 4256, 'UNCLASSIFIED': 5125}
    expected_classes = {'CONTEXTUAL': 1184, 'NON_SEXUAL': 23745, 'NULL': 5125, 'SEXUAL': 1698}
    ...
    summary = {
        'issue': 118,
        'mode': 'RESEARCH_SIDECAR_V39_V64_FULL_REVIEW',
