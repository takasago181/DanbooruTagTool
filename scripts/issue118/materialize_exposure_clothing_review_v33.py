...
OUT=Path('docs/issue118/exposure_clothing_review_v33')
LEX_GROUPS={
...
def main():
    g={norm(r['canonical']):r for r in read_csv(GENERAL)};side=read_csv(SIDECAR);rows=[]
    for r in side:
        if r['review_status']!='UNCLASSIFIED' or r.get('is_special')=='YES':continue
        key=r['identity_key'];gr=g.get(key)
        if gr is None or (gr.get('primary_path') or '').strip()!='CLOTHING_STATE_EXPOSURE' or flags(key)!='SEXUALIZED_CLOTHING':continue
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_EXPOSURE_CLOTHING_V33','human_intent':'','review_note':''})
    rows.sort(key=lambda r:r['identity_key'])
    if len(rows)!=83:raise SystemExit(f'expected 83 General-only CLOTHING_STATE_EXPOSURE SEXUALIZED_CLOTHING rows, got {len(rows)}')
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'full_review_candidate_v33.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    s={'issue':118,'mode':'EXPOSURE_CLOTHING_FULL_REVIEW_V33','candidate_rows':83,'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 83 rows'}
    (OUT/'summary_v33.json').write_text(json.dumps(s,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(s,ensure_ascii=False,sort_keys=True));return 0
