...
SIDECAR=Path('docs/issue118/research_sidecar_v90.csv')
OUT=Path('docs/issue118/general_none_none_review_v124')
EXPECTED_CLUSTER_ROWS=439
EXPECTED_ELIGIBLE_ROWS=437
BATCH_ROWS=100
RETAINED_EXCLUSIONS={'insertion_threshold_(meme)','lilistia'}
LEX_GROUPS={
...
def main():
    ...
    for r in side:
        ...
        if ((gr or {}).get('primary_path') or '').strip():continue
        if flags(key)!='NONE':continue
        pool.append(key)
    pool=sorted(set(pool))
    if len(pool)!=EXPECTED_CLUSTER_ROWS: raise SystemExit(f'expected {EXPECTED_CLUSTER_ROWS} giant-cluster rows, got {len(pool)}')
    eligible=[k for k in pool if k not in RETAINED_EXCLUSIONS]
    if len(eligible)!=EXPECTED_ELIGIBLE_ROWS: raise SystemExit(f'expected {EXPECTED_ELIGIBLE_ROWS} eligible giant-cluster rows, got {len(eligible)}')
    chosen=eligible[:BATCH_ROWS]
    rows=[{'identity_key':k,'candidate_cluster':'GENERAL_ONLY|(none)|NONE','candidate_state':'FULL_REVIEW_GENERAL_NONE_NONE_V124_BATCH19','human_intent':'','review_note':''} for k in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v124.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary={'issue':118,'mode':'GENERAL_NONE_NONE_FULL_REVIEW_V124_BATCH19','source_sidecar':'research_sidecar_v90.csv','source_cluster':'GENERAL_ONLY|(none)|NONE',
      'source_cluster_rows':len(pool),'retained_exclusions':sorted(RETAINED_EXCLUSIONS),'eligible_cluster_rows':len(eligible),'candidate_rows':len(rows),
      'selection':'alphabetically first 100 identities from remaining fixed giant General-only no-path no-lex cluster after V123, excluding previously retained unresolved identities; selection only, not classification authority',
      'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','review_verdicts_generated_by_materializer':'NO',
      'review_artifacts_are_external_inputs':'YES','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO',
      'next_gate':'full manual review of all 100 rows'}
    (OUT/'summary_v124.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
