...
SIDECAR = Path('docs/issue118/research_sidecar_v56.csv')
OUT = Path('docs/issue118/general_action_interaction_none_review_v82')
EXPECTED_CLUSTER_ROWS = 95
BATCH_ROWS = 95
...
def main() -> int:
    ...
    rows=[{
        ...
        'candidate_state':'FULL_REVIEW_GENERAL_ACTION_INTERACTION_NONE_V82_BATCH8',
        'human_intent':'',
        'review_note':'',
    } for key in chosen]
    ...
    with (OUT/'full_review_candidate_v82.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n')
        w.writeheader(); w.writerows(rows)

    summary={
        'issue':118,
        'mode':'GENERAL_ACTION_INTERACTION_NONE_FULL_REVIEW_V82_BATCH8',
        'source_sidecar':'research_sidecar_v56.csv',
        'source_cluster':'GENERAL_ONLY|ACTION_CONTACT/INTERACTION|NONE',
        'source_cluster_rows':len(cluster),
        'candidate_rows':len(rows),
        'batch_selection':'all 95 remaining identities from the fixed General-only Action/Interaction cluster after V81; selection only, not classification authority',
    ...
    (OUT/'summary_v82.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
