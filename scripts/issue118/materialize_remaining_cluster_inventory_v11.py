...
OUT = Path('docs/issue118/remaining_cluster_inventory_v11')
...
def flags(key: str) -> str:
    ...
    if re.search(r'(?:^|[_/\-])top[/_-]bottom[_/-]dynamic(?:$|[_/\-])', key.lower()):
        hit.append('RELATIONSHIP_ROLE')
    if re.search(r'(?:^|[_/\-])bottom[/_-]top[_/-]dynamic(?:$|[_/\-])', key.lower()):
        hit.append('RELATIONSHIP_ROLE')
    hit = sorted(set(hit))
...
def main() -> int:
    ...
    if len(rows) != 14040:
        raise SystemExit(f'expected 14040 remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    with (OUT / 'cluster_inventory_v11.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V11_FROM_SIDECAR_V4',
        'source_sidecar': 'research_sidecar_v4.csv',
    ...
    (OUT / 'summary_v11.json').write_text(
