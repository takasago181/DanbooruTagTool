...
OUT = Path('docs/issue118/intimate_action_review_v31')
...
def main() -> int:
    ...
    for r in side:
        ...
        if gr is None or (gr.get('primary_path') or '').strip() != 'ACTION_CONTACT/INTIMATE':
            continue
        if flags(key) != 'NONE':
            continue
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_INTIMATE_ACTION_V31','human_intent':'','review_note':''})
    rows.sort(key=lambda r: r['identity_key'])
    if len(rows) != 80:
        raise SystemExit(f'expected 80 General-only ACTION_CONTACT/INTIMATE NONE rows, got {len(rows)}')
    OUT.mkdir(parents=True, exist_ok=True)
    with (OUT / 'full_review_candidate_v31.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['identity_key','candidate_state','human_intent','review_note'], lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary = {'issue':118,'mode':'INTIMATE_ACTION_FULL_REVIEW_V31','candidate_rows':len(rows),'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 80 rows'}
    (OUT / 'summary_v31.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
