...
SIDECAR = Path('docs/issue118/research_sidecar_v42.csv')
OUT = Path('docs/issue118/general_body_part_none_review_v68')
EXPECTED_CLUSTER_ROWS = 43
EXCLUDED_RETAINED = {'knee_boobs','powerful_ass'}
EXPECTED_NEW_ROWS = 41
...
def main()->int:
    ...
    if not EXCLUDED_RETAINED <= set(cluster):
        raise SystemExit('retained exclusions missing from source cluster')
    chosen=[k for k in cluster if k not in EXCLUDED_RETAINED]
    if len(chosen)!=EXPECTED_NEW_ROWS:
        raise SystemExit(f'expected {EXPECTED_NEW_ROWS} new rows, got {len(chosen)}')
    rows=[{
        'identity_key':key,
        'candidate_cluster':'GENERAL_ONLY|BODY_PART|NONE',
        'candidate_state':'FULL_REVIEW_GENERAL_BODY_PART_NONE_V68_BATCH3',
        'human_intent':'',
        'review_note':'',
    } for key in chosen]
    OUT.mkdir(parents=True,exist_ok=True)
    fields=['identity_key','candidate_cluster','candidate_state','human_intent','review_note']
    with (OUT/'full_review_candidate_v68.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields,lineterminator='\n')
        w.writeheader(); w.writerows(rows)
    summary={
        'issue':118,
        'mode':'GENERAL_BODY_PART_NONE_FULL_REVIEW_V68_BATCH3',
        'source_sidecar':'research_sidecar_v42.csv',
        'source_cluster':'GENERAL_ONLY|BODY_PART|NONE',
        'source_cluster_rows':len(cluster),
        'retained_exclusions':sorted(EXCLUDED_RETAINED),
        'candidate_rows':len(rows),
        'batch_selection':'all remaining identities except prior intentionally retained UNCLASSIFIED rows',
        ...
        'catalog_mutated':'NO',
        'user_db_mutated':'NO',
        'next_gate':'full manual review of all 41 new rows',
    }
    (OUT/'summary_v68.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
