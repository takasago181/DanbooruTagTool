...
BASE = Path('docs/issue118/research_sidecar_v39.csv')
REVIEW = Path('docs/issue118/general_clothing_everyday_none_review_v65/review_policy_v65.json')
CANDIDATE = Path('docs/issue118/general_clothing_everyday_none_review_v65/full_review_candidate_v65.csv')
REVIEW_BLOB = 'e8acbf3d522f105f4f390378265166de0395b9cb'
CANDIDATE_BLOB = '2d440af6c670845c0796be949b65f2336bf71f32'
OUT = Path('docs/issue118/research_sidecar_v40.csv')
SUMMARY = Path('docs/issue118/research_sidecar_summary_v40.json')
...
def main() -> int:
    if git_blob_sha(REVIEW) != REVIEW_BLOB:
        raise SystemExit('V65 fixed review blob SHA drift')
    if git_blob_sha(CANDIDATE) != CANDIDATE_BLOB:
        raise SystemExit('V65 fixed candidate blob SHA drift')
    ...
    if len(source) != 60 or review.get('source_rows') != 60:
        raise SystemExit('V65 source count drift')
    if review.get('review_verdicts_generated_by_materializer') != 'NO':
        raise SystemExit('V65 provenance drift: generated verdicts')
    if review.get('review_artifacts_are_external_inputs') != 'YES':
        raise SystemExit('V65 provenance drift: external input flag')
    ...
    if review.get('unclassified'):
        raise SystemExit('V65 has unexpected unclassified rows')
    ...
    for intent, keys in groups.items():
        for key in sorted(keys):
            ...
            if target is None or target['review_status'] != 'UNCLASSIFIED' or target['sexual_intent']:
                raise SystemExit(f'V65 not cleanly unclassified: {key}')
            target['sexual_intent'] = intent
            target['review_status'] = 'HUMAN_REVIEWED'
            target['rule_id'] = f'HUMAN_{intent}_V65'
            target['evidence'] = 'full semantic review of fixed V65 candidate set'
            promoted[intent] += 1
    ...
    expected_status = {'AUTO_HIGH_CONF': 22371, 'HUMAN_REVIEWED': 4316, 'UNCLASSIFIED': 5065}
    expected_classes = {'CONTEXTUAL': 1190, 'NON_SEXUAL': 23799, 'NULL': 5065, 'SEXUAL': 1698}
    ...
    summary = {
        'issue': 118,
        'mode': 'RESEARCH_SIDECAR_V40_V65_FULL_REVIEW',
