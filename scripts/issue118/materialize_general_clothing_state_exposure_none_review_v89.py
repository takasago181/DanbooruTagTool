...
SIDECAR = Path('docs/issue118/research_sidecar_v62.csv')
OUT = Path('docs/issue118/general_clothing_state_exposure_none_review_v89')
EXPECTED_CLUSTER_ROWS = 133
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows = [{
        ...
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_STATE_EXPOSURE_NONE_V89_BATCH6',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v89.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_STATE_EXPOSURE_NONE_FULL_REVIEW_V89_BATCH6',
        'source_sidecar': 'research_sidecar_v62.csv',
        'source_cluster': 'GENERAL_ONLY|CLOTHING_STATE_EXPOSURE|NONE',
        'source_cluster_rows': len(cluster),
        'candidate_rows': len(rows),
        'batch_selection': 'alphabetically first 100 identities from the remaining fixed General-only clothing-state exposure cluster after V88; selection only, not classification authority',
    ...
    (OUT / 'summary_v89.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
