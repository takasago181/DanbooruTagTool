...
OUT=Path('docs/issue118/overlap_restraint_action_review_v50')
LEX_GROUPS={
...
def main():
    ...
    for r in side:
        ...
        if (sr.get('GenerationFamily') or '').strip()!='RESTRAINT_ACTION':continue
        if flags(key)!='NONE':continue
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_OVERLAP_RESTRAINT_ACTION_V50','human_intent':'','review_note':''})
    rows.sort(key=lambda r:r['identity_key'])
    if len(rows)!=38:raise SystemExit(f'expected 38 rows, got {len(rows)}')
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'full_review_candidate_v50.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    s={'issue':118,'mode':'OVERLAP_RESTRAINT_ACTION_FULL_REVIEW_V50','candidate_rows':38,'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 38 rows'}
    (OUT/'summary_v50.json').write_text(json.dumps(s,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(s,ensure_ascii=False,sort_keys=True));return 0
