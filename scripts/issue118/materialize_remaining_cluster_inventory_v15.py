...
OUT = Path('docs/issue118/remaining_cluster_inventory_v15')
...
def main() -> int:
    ...
    if len(rows) != 10728:
        raise SystemExit(f'expected 10728 remaining UNCLASSIFIED rows, got {len(rows)}')
    ...
    with (OUT / 'cluster_inventory_v15.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=fields, lineterminator='\n')
        w.writeheader(); w.writerows(inventory)

    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V15_FROM_SIDECAR_V6',
        'source_sidecar': 'research_sidecar_v6.csv',
    ...
    (OUT / 'summary_v15.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
