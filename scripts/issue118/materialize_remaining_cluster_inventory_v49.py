...
SIDECAR=Path('docs/issue118/research_sidecar_v28.csv')
OUT=Path('docs/issue118/remaining_cluster_inventory_v49')
LEX_GROUPS={
...
def main():
    general=read_csv(GENERAL);special=read_csv(SPECIAL);side=read_csv(SIDECAR);g={norm(r['canonical']):r for r in general};s={norm(r['Tag']):r for r in special}
    clusters=defaultdict(list);membership_counts=Counter();root_counts=Counter();path_counts=Counter();family_counts=Counter();lex_counts=Counter();rows=[]
    for r in side:
        ...
        path=((gr or {}).get('primary_path') or '').strip() or '(none)';rt=root(path if path!='(none)' else '')
        fam=((sr or {}).get('GenerationFamily') or '').strip() or '(none)';role=((sr or {}).get('GenerationRole') or '').strip() or '(none)';fl=flags(key)
    ...
    if len(rows)!=6017:raise SystemExit(f'expected 6017 remaining UNCLASSIFIED rows, got {len(rows)}')
    inventory=[]
    for ck,items in clusters.items():
        a=items[0];inventory.append({'cluster_key':ck,'rows':len(items),'membership':a['membership'],'general_path':a['general_path'],'general_root':a['general_root'],'generation_family':a['generation_family'],'lex_flags':a['lex_flags'],'examples':';'.join(x['identity_key'] for x in sorted(items,key=lambda x:x['identity_key'])[:12])})
