...
OUT = Path('docs/issue118/special_exposure_review_v30')
...
def main() -> int:
    ...
    for r in side:
        ...
        if g.get(key) is not None:
            continue
        if flags(key) != 'EXPOSURE_INTIMATE':
            continue
        rows.append({'identity_key': key, 'candidate_state': 'FULL_REVIEW_SPECIAL_EXPOSURE_V30', 'human_intent': '', 'review_note': ''})
    rows.sort(key=lambda r: r['identity_key'])
    if len(rows) != 78:
        raise SystemExit(f'expected 78 Special-only EXPOSURE_INTIMATE rows, got {len(rows)}')
    OUT.mkdir(parents=True, exist_ok=True)
    with (OUT / 'full_review_candidate_v30.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['identity_key','candidate_state','human_intent','review_note'], lineterminator='\n'); w.writeheader(); w.writerows(rows)
    summary = {'issue':118,'mode':'SPECIAL_EXPOSURE_FULL_REVIEW_V30','candidate_rows':len(rows),'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 78 rows'}
    (OUT / 'summary_v30.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
