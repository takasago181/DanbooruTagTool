...
OUT = Path('docs/issue118/clothing_everyday_exposure_review_v34')
...
def main() -> int:
    ...
    for r in side:
        ...
        if flags(key) != 'EXPOSURE_INTIMATE':
            continue
        rows.append({'identity_key': key, 'candidate_state': 'FULL_REVIEW_CLOTHING_EVERYDAY_EXPOSURE_V34', 'human_intent': '', 'review_note': ''})
    rows.sort(key=lambda r: r['identity_key'])
    if len(rows) != 180:
        raise SystemExit(f'expected 180 rows, got {len(rows)}')
    OUT.mkdir(parents=True, exist_ok=True)
    with (OUT / 'full_review_candidate_v34.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['identity_key','candidate_state','human_intent','review_note'], lineterminator='\n')
        w.writeheader(); w.writerows(rows)
    summary = {
        'issue': 118,
        'mode': 'CLOTHING_EVERYDAY_EXPOSURE_FULL_REVIEW_V34',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 180 rows'
    }
    (OUT / 'summary_v34.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
