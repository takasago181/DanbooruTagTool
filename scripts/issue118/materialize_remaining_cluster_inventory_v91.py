...
OUT = Path('docs/issue118/remaining_cluster_inventory_v91')
EXPECTED_REMAINING = 2973
...
def main() -> int:
    ...
    with (OUT / 'cluster_inventory_v91.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V91_FROM_SIDECAR_V64',
    ...
    (OUT / 'summary_v91.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
