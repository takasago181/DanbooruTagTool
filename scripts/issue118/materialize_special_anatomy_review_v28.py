...
SIDECAR = Path('docs/issue118/research_sidecar_v15.csv')
OUT = Path('docs/issue118/special_anatomy_review_v28')
...
def main() -> int:
    ...
    g = {norm(r['canonical']) for r in general}
    ...
    for r in side:
        ...
        if fam:
            continue
        if flags(key) != 'ANATOMY':
            continue
        rows.append({'identity_key': key, 'candidate_state': 'FULL_REVIEW_SPECIAL_ANATOMY_V28', 'human_intent': '', 'review_note': ''})

    rows.sort(key=lambda r: r['identity_key'])
    if len(rows) != 170:
        raise SystemExit(f'expected 170 Special-only ANATOMY rows, got {len(rows)}')

    OUT.mkdir(parents=True, exist_ok=True)
    with (OUT / 'full_review_candidate_v28.csv').open('w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['identity_key','candidate_state','human_intent','review_note'], lineterminator='\n')
        w.writeheader(); w.writerows(rows)
    summary = {
        'issue': 118,
        'mode': 'SPECIAL_ANATOMY_FULL_REVIEW_V28',
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 170 rows'
    }
    (OUT / 'summary_v28.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
