...
OUT=Path('docs/issue118/action_object_use_review_v44')
...
def main():
    ...
    for r in side:
        ...
        if (gr.get('primary_path') or '').strip()!='ACTION_CONTACT/OBJECT_USE':continue
        if flags(key)!='NONE':continue
        rows.append({'identity_key':key,'candidate_state':'FULL_REVIEW_ACTION_OBJECT_USE_V44','human_intent':'','review_note':''})
    rows.sort(key=lambda r:r['identity_key'])
    if len(rows)!=49:raise SystemExit(f'expected 49 rows, got {len(rows)}')
    OUT.mkdir(parents=True,exist_ok=True)
    with (OUT/'full_review_candidate_v44.csv').open('w',encoding='utf-8',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    summary={'issue':118,'mode':'ACTION_OBJECT_USE_FULL_REVIEW_V44','candidate_rows':49,'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 49 rows'}
    (OUT/'summary_v44.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(summary,ensure_ascii=False,sort_keys=True));return 0
