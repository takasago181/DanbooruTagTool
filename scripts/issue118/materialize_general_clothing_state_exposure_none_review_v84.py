...
OUT = Path('docs/issue118/general_clothing_state_exposure_none_review_v84')
EXPECTED_CLUSTER_ROWS = 633
BATCH_ROWS = 100
...
def main() -> int:
    ...
    rows = [{
        ...
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_STATE_EXPOSURE_NONE_V84_BATCH1',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v84.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_STATE_EXPOSURE_NONE_FULL_REVIEW_V84_BATCH1',
        ...
        'batch_selection': 'alphabetically first 100 identities from the fixed General-only clothing-state exposure cluster; selection only, not classification authority',
    ...
    (OUT / 'summary_v84.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
