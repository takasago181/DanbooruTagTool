...
OUT = Path('docs/issue118/remaining_cluster_inventory_v93')
EXPECTED_REMAINING = 2876
...
def main() -> int:
    ...
    with (OUT / 'cluster_inventory_v93.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V93_FROM_SIDECAR_V65',
    ...
    (OUT / 'summary_v93.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
