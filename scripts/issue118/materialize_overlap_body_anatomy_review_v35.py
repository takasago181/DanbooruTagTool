...
SIDECAR = Path('docs/issue118/research_sidecar_v21.csv')
OUT = Path('docs/issue118/overlap_body_anatomy_review_v35')
...
def anatomy_only(key: str) -> bool:
    parts = {p for p in re.split(r'[_()\-/]+', key.lower()) if p}
    if not (parts & ANATOMY):
        return False
    if any(parts & terms for terms in OTHER.values()):
        return False
    return True
...
def main() -> int:
    ...
    for r in side:
        ...
        if (gr.get('primary_path') or '').strip() != 'BODY_PART':
            continue
        fam = (sr.get('GenerationFamily') or '').strip()
        if fam:
            continue
        if not anatomy_only(key):
            continue
        rows.append({'identity_key': key, 'candidate_state': 'FULL_REVIEW_OVERLAP_BODY_ANATOMY_V35', 'human_intent': '', 'review_note': ''})
    rows.sort(key=lambda r: r['identity_key'])
    if len(rows) != 66:
        raise SystemExit(f'expected 66 rows, got {len(rows)}')
    OUT.mkdir(parents=True, exist_ok=True)
    with (OUT / 'full_review_candidate_v35.csv').open('w', encoding='utf-8', newline='') as f:
        w=csv.DictWriter(f,fieldnames=['identity_key','candidate_state','human_intent','review_note'],lineterminator='\n');w.writeheader();w.writerows(rows)
    summary={'issue':118,'mode':'OVERLAP_BODY_ANATOMY_FULL_REVIEW_V35','candidate_rows':len(rows),'selection_rules_use':'DISCOVERY_ONLY_NOT_CLASSIFICATION_AUTHORITY','auto_promotion_performed':'NO','production_authority':'NO','main_mutated':'NO','issue117_code_mutated':'NO','catalog_mutated':'NO','user_db_mutated':'NO','next_gate':'full manual review of all 66 rows'}
    (OUT/'summary_v35.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
