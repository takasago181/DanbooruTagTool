...
TARGETS = {
    ('CLOTHING_STATE_EXPOSURE','BODY_ATTRIBUTE','ANATOMY'): ('OVERLAP|CLOTHING_STATE_EXPOSURE|BODY_ATTRIBUTE|ANATOMY', 17),
    ('CLOTHING','CLOTHING_EXPOSURE','SEXUALIZED_CLOTHING'): ('OVERLAP|CLOTHING|CLOTHING_EXPOSURE|SEXUALIZED_CLOTHING', 17),
    ('CLOTHING_STATE_EXPOSURE','(none)','EXPOSURE_INTIMATE'): ('OVERLAP|CLOTHING_STATE_EXPOSURE|(none)|EXPOSURE_INTIMATE', 16),
    ('CLOTHING','CLOTHING_EXPOSURE','EXPOSURE_INTIMATE'): ('OVERLAP|CLOTHING|CLOTHING_EXPOSURE|EXPOSURE_INTIMATE', 14),
    ('CLOTHING_STATE_EXPOSURE','CLOTHING_EXPOSURE','ANATOMY'): ('OVERLAP|CLOTHING_STATE_EXPOSURE|CLOTHING_EXPOSURE|ANATOMY', 13),
    ('ACTION_CONTACT','CLOTHING_EXPOSURE','ANATOMY'): ('OVERLAP|ACTION_CONTACT|CLOTHING_EXPOSURE|ANATOMY', 12),
}
...
def main() -> int:
    ...
    for r in side:
        ...
        rows.append({
            ...
            'candidate_state': 'FULL_REVIEW_EXPOSURE_SIX_CLUSTER_V60',
            'human_intent': '',
            'review_note': '',
        })
    ...
    if len(rows) != 89:
        raise SystemExit(f'expected 89 rows, got {len(rows)}')
    ...
    with (OUT / 'full_review_candidate_v60.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        ...
        'catalog_mutated': 'NO',
        'user_db_mutated': 'NO',
        'next_gate': 'full manual review of all 89 rows',
    }
    (OUT / 'summary_v60.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
