...
SIDECAR = Path('docs/issue118/research_sidecar_v60.csv')
OUT = Path('docs/issue118/general_clothing_state_exposure_none_review_v87')
EXPECTED_CLUSTER_ROWS = 333
BATCH_ROWS = 100
...
def main() -> int:
    ...
    for r in side:
        ...
        if gr is None:
            continue
        if (gr.get('primary_path') or '').strip() != 'CLOTHING_STATE_EXPOSURE':
            continue
        if flags(key) != 'NONE':
            continue
        cluster.append(key)
    ...
    rows = [{
        'identity_key': key,
        'candidate_cluster': 'GENERAL_ONLY|CLOTHING_STATE_EXPOSURE|NONE',
        'candidate_state': 'FULL_REVIEW_GENERAL_CLOTHING_STATE_EXPOSURE_NONE_V87_BATCH4',
        'human_intent': '',
        'review_note': '',
    } for key in chosen]
    ...
    with (OUT / 'full_review_candidate_v87.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'GENERAL_CLOTHING_STATE_EXPOSURE_NONE_FULL_REVIEW_V87_BATCH4',
        'source_sidecar': 'research_sidecar_v60.csv',
        'source_cluster': 'GENERAL_ONLY|CLOTHING_STATE_EXPOSURE|NONE',
        'source_cluster_rows': len(cluster),
        'candidate_rows': len(rows),
        'batch_selection': 'alphabetically first 100 identities from the remaining fixed General-only clothing-state exposure cluster after V86; selection only, not classification authority',
    ...
    (OUT / 'summary_v87.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
