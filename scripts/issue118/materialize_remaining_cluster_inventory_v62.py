...
SIDECAR = Path('docs/issue118/research_sidecar_v37.csv')
OUT = Path('docs/issue118/remaining_cluster_inventory_v62')
EXPECTED_REMAINING = 5325
...
def main() -> int:
    ...
    with (OUT / 'cluster_inventory_v62.csv').open('w', encoding='utf-8', newline='') as f:
    ...
    summary = {
        'issue': 118,
        'mode': 'REMAINING_CLUSTER_INVENTORY_V62_FROM_SIDECAR_V37',
        'source_sidecar': 'research_sidecar_v37.csv',
    ...
    (OUT / 'summary_v62.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    print(json.dumps({
        'remaining_unclassified_rows': len(rows),
        'membership_counts': dict(sorted(membership_counts.items())),
        'cluster_count': len(inventory),
