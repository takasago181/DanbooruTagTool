#!/usr/bin/env python3
"""Fresh evidence diagnostic for all remaining REVIEW_REQUIRED Character rows.
...
ROOT=Path(__file__).resolve().parents[2];AUDIT=ROOT/'docs/issue70/audit';TMP=ROOT/'artifacts/issue70-batch060';OUT=AUDIT/'character_review_remaining_batch060_diagnostic.json'
...
def main():
    audited=done();rows=[];buckets=Counter();sig=Counter();samples=defaultdict(list);strong_fields=('existing_display_ja','existing_search_ja','verified_aliases','source_aliases')
    fields=['row_id','canonical_tag','post_count','impact_tier','display_ja','search_ja','translation_status','translation_note','risk_flags','source_aliases','verified_aliases','existing_display_ja','existing_search_ja','existing_candidate_ja','existing_rejected_ja','related_copyright_top1','related_copyright_top1_coverage','family_base_canonical','family_base_display_ja']
    for r in census():
        if r.get('row_id') in audited or r.get('category_name')!='Character' or r.get('translation_status')!='REVIEW_REQUIRED':continue
        ...
        exact=[(f,t) for f,t in strong if norm(t)==nd];raw=[t for t in terms(r.get('existing_candidate_ja') or '') if JA.search(t)];raw_exact=any(norm(t)==nd for t in raw);family=bool((r.get('family_base_canonical') or '').strip() and (r.get('family_base_display_ja') or '').strip());flags={x.strip() for x in (r.get('risk_flags') or '').split('|') if x.strip()};identity=bool({'DUPLICATE_DISPLAY_WITHIN_CATEGORY','DISAMBIGUATOR_NOT_VISIBLE','VARIANT_DISPLAY_MISSING_BASE_IDENTITY','RAW_TAG_SYNTAX_IN_DISPLAY'} & flags)
        ...
        item={k:r.get(k,'') for k in fields};item.update({'diagnostic_bucket':bucket,'strong_exact_terms':' | '.join(f'{f}:{t}' for f,t in exact),'strong_ja_terms':' | '.join(f'{f}:{t}' for f,t in strong),'raw_candidate_exact':raw_exact});rows.append(item)
    ...
    OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print({'population':len(rows),'bucket_counts':dict(buckets),'production_modified':False})
