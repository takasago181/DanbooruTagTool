...
def main() -> None:
    ...
    for path in sorted(AUDIT.glob("*.csv")):
        ...
        for r in rows:
            ...
            category = (r.get("category") or r.get("category_name") or root.get("category") or "").strip()
            if not is_overlay:
    ...
    for rid, b in base.items():
        ...
        if eff["audit_verdict"] != "NEEDS_EXTERNAL_CHECK":
            continue
        root = root_by_id.get(rid, {})
        category = b["category"] or (root.get("category") or "").strip()
        if category not in VALID_CATEGORIES:
            category = infer_category(category, b["source_file"])
        unresolved.append({
            "row_id": b["row_id"],
