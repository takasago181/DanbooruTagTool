...
def main() -> None:
    ...
    for c in top:
        rr = root.get(c["row_id"])
        if not rr or (rr.get("category") or "").strip() != "Copyright":
            continue
    ...
    fields_all = ["row_id","canonical_tag","post_count","display_ja","search_ja","translation_note","reason_code","source_file","effective_source_file"]
    ...
    with OUT_COPYRIGHT.open("w", encoding="utf-8-sig", newline="") as f:
        ...
        w.writeheader(); w.writerows(copyright_rows)
