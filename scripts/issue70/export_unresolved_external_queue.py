...
OUT = AUDIT / 'EXTERNAL_QUEUE_TOP100.csv'
SUMMARY = AUDIT / 'EXTERNAL_QUEUE_SUMMARY.json'
...
def main():
    ...
    for path in sorted(AUDIT.glob('*.csv')):
        ...
        try:
            ...
            continue
        for r in rows:
            rid=(r.get('row_id') or '').strip(); verdict=(r.get('audit_verdict') or '').strip()
    ...
    with OUT.open('w',encoding='utf-8-sig',newline='') as f:
        w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); w.writerows(unresolved[:100])
    SUMMARY.write_text(json.dumps({'unresolved_external':len(unresolved),'top100_rows':min(100,len(unresolved))},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print({'unresolved_external':len(unresolved),'top100_rows':min(100,len(unresolved)),'production_modified':False})
