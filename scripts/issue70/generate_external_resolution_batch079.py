...
CANDIDATES = Path("docs/issue70/audit/EXTERNAL_QUEUE_BATCH079_COPYRIGHT_CANDIDATES.csv")
SHORTLISTS = [
    Path("docs/issue70/audit/BATCH079_VERIFIED_SHORTLIST_A.csv"),
    Path("docs/issue70/audit/BATCH079_VERIFIED_SHORTLIST_B.csv"),
    Path("docs/issue70/audit/BATCH079_VERIFIED_SHORTLIST_C.csv"),
]
OUT = Path("docs/issue70/audit/external_resolution_copyright_official_batch079.csv")
...
def main() -> None:
    ...
    assert len(set(tags)) == 80, "duplicate canonical_tag across batch079 shortlists"
    ...
    assert not missing_effective, f"batch079 targets not effective unresolved: {missing_effective}"
    missing_queue = sorted(set(tags) - set(by_tag))
    assert not missing_queue, f"batch079 targets absent from root queue: {missing_queue}"
    ...
    for s in shortlist_rows:
        ...
        rows.append({
            ...
            "reason_code": "OFFICIAL_OR_FIRST_PARTY_TITLE_CONFIRMED_BATCH079",
            "confidence": "HIGH",
