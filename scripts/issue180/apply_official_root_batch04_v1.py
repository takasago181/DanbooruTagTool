#!/usr/bin/env python3
"""Issue #180 official-root authority batch 04.

Research-only / fail-closed.
Takes a manually reviewed set of remaining IP qualifier families, validates
their canonical Copyright roots against the accepted catalog, and withholds
explicit nested variants from automatic HOME confirmation.
"""
import csv
import json
import re
...
EV = R / "docs/issue180/evidence/official_root_authority_batch04.csv"
OUT = D / "OFFICIAL_ROOT_BATCH04_V1.csv"
SUMMARY = D / "official_root_batch04_v1_summary.json"
...
def main():
    ...
    if any(r.get("root_review") != "PASS" for r in evidence_rows):
        raise SystemExit("batch04 evidence contains non-PASS root")
    ...
    missing_roots = sorted(
        e["home_copyright"] for e in evidence_rows
        if e["home_copyright"] not in copyright_roots
    )
    if missing_roots:
    ...
    if len(ip_rows) != EXPECTED_ROWS:
        raise SystemExit(f"batch04 population drift: expected {EXPECTED_ROWS}, got {len(ip_rows)}")

    actual_counts = {}
    for r in ip_rows:
        f = r["final_qualifier"]
        actual_counts[f] = actual_counts.get(f, 0) + 1
    ...
    if (pass_rows, variant_rows) != (EXPECTED_PASS, EXPECTED_VARIANTS):
        raise SystemExit(
            f"batch04 pass/variant drift: expected {(EXPECTED_PASS, EXPECTED_VARIANTS)}, "
