...
MASTER=D/"MASTER_HOME/CHARACTER_HOME_MASTER_V1.csv"
OUT=D/"MASTER_HOME/UNRESOLVED_PRIORITY_V1.csv"
...
def main():
 master=read(MASTER)
 unresolved={r["canonical_tag"] for r in master if r["final_state"]=="HOME_UNRESOLVED"}
 meta={}
 ...
 for tag in sorted(unresolved):
  ...
  out.append({"canonical_tag":tag,"priority":priority,"queue_kind":kind,"final_qualifier":qualifier,"roster_candidate_state":x.get("roster_candidate_state",""),"final_state":"HOME_UNRESOLVED"})
 with OUT.open("w",encoding="utf-8-sig",newline="") as f:
  w=csv.DictWriter(f,fieldnames=out[0].keys(),lineterminator=chr(10))
  w.writeheader()
  w.writerows(out)
 pc=Counter(r["priority"] for r in out)
 kc=Counter(r["final_qualifier"] for r in out if r["priority"]=="P2_VARIANT_OR_ATTRIBUTE" and r["final_qualifier"])
 qc=Counter(r["final_qualifier"] for r in out if r["priority"]=="P1_FAMILY_AUTHORITY" and r["final_qualifier"])
 summary={"unresolved_rows":len(out),"priority_counts":dict(pc),"top_variant_attribute_families":kc.most_common(100),"top_ip_families":qc.most_common(100),"master_unresolved_match":len(out)==len(unresolved),"auto_approved":0,"production_modified":False}
 (D/"MASTER_HOME/unresolved_priority_v1_summary.json").write_text(json.dumps(summary,ensure_ascii=False,indent=2)+chr(10),encoding="utf-8")
