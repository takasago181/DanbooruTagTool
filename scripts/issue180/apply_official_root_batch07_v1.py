#!/usr/bin/env python3
"""Issue #180 official-root authority batch 07.
...
EV=R/"docs/issue180/evidence/official_root_authority_batch07.csv"
OUT=D/"OFFICIAL_ROOT_BATCH07_V1.csv"
SUMMARY=D/"official_root_batch07_v1_summary.json"
...
def main():
    ...
    if any(r.get("root_review")!="PASS" for r in evrows):
        raise SystemExit("batch07 evidence contains non-PASS root")
    ...
    if len(rows)!=EXPECTED_ROWS:
        raise SystemExit(f"batch07 population drift: expected {EXPECTED_ROWS}, got {len(rows)}")
    counts={}
    ...
    if (passed,variants)!=(EXPECTED_PASS,EXPECTED_VARIANTS):
        raise SystemExit(f"batch07 pass/variant drift: expected {(EXPECTED_PASS,EXPECTED_VARIANTS)}, got {(passed,variants)}")
