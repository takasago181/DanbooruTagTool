...
def test_full_rollout_validates_against_frozen_2788_source():
    ...
    meta = validate_mapping(rows, mappings, genres, subgenres, require_complete=True)

    assert len(rows) == 2788
    assert {row.special_id for row in rows} == set(range(1, 2789))
    assert meta["mapped_total"] == 2788
    assert meta["unmapped_total"] == 0
    assert meta["old_other_total"] == sum(
        1 for row in rows if row.source_file.startswith(OTHER_PREFIX)
    )
    assert meta["old_other_mapped"] == meta["old_other_total"]
    assert meta["old_other_unmapped"] == 0


def test_full_rollout_builds_zero_old_other_review_queue(tmp_path):
    meta = build_outputs(tmp_path, require_complete=True)
    ...
    combined = _read_csv(partial_path)
    saved_meta = json.loads(meta_path.read_text(encoding="utf-8"))

    assert queue == []
    assert len(combined) == 2788
    assert meta["mapped_total"] == 2788
    assert meta["unmapped_total"] == 0
    assert saved_meta["mapped_total"] == 2788
    assert saved_meta["unmapped_total"] == 0


def test_require_complete_accepts_all_2788_reviewed_rows():
    rows, _ = load_prompt_reference(ROOT / "data" / "special2788" / "prompt_reference")
    _, genres, subgenres = load_taxonomy()
    mappings, _ = merge_mapping_sources()
    meta = validate_mapping(rows, mappings, genres, subgenres, require_complete=True)
