...
public sealed class SpecialBrowseV2Index
{
    ...
    public int Count(SpecialBrowseV2Filter filter)
        => byCatalogId.Values
            .Where(entry => entry.CanBrowse && Matches(entry.CatalogId, filter))
            .Select(entry => entry.CatalogId)
            .DistinctBy(catalogId => byCatalogId[catalogId] is { Canonical: { } canonical } ? "C:" + canonical : "S:" + catalogId, StringComparer.Ordinal)
            .Count();
