...
Archived from `docs/project/CURRENT_STATE.md` during Issue #188 execution-overhead cleanup.

This file preserves historical routing snapshots, completed-lane evidence, old runtime baselines, accepted taxonomy summaries, and prior state notes.

**Do not use this file as current routing authority.**
...
- **#188**: project-wide execution-overhead改善lane。#132/#179/#180のsemantic authorityを奪わず、cold/warm resume分離、compact routing、status=cache原則、CI tiering、large-source transport、efficiency lintを整備する。active semantic researchを停止させない。
- #132の現行運用正本は research branch `research/taxonomy-usability-audit:docs/issue132/parallel/CURRENT_AUTOMATION_OPERATION.md`。compact worker cacheも同branchの `docs/issue132/parallel/WORKER_EXECUTION_CARD_V1.md`。mainへ運用文書を複製して二重正本化しない。**300 identities / worker run は上限でノルマではない**。通常runは `WORKER_EXECUTION_CARD_V1.md` のcompact preflightを使い、6万文字規模のfrozen docs再読を毎回行わない。新規workは **25-row immutable checkpoint**。各identityを次へ進む前に厳格finalizeし、曖昧・固有名詞・専門語・性的/身体境界例は推測せず `RESEARCHED`、未解決なら `SEMANTIC_UNRESOLVED`。lane-local 100-row境界でhigh-risk全件＋ordinary CHECKEDの決定論的spot-checkを行う。raw throughputだけを品質PASSとしない。
...
- #132 progress snapshot at this sync: valid persisted checkpoint union **500 / 31,003** (Lane 1: 225, Lane 2: 175, Lane 3: 100)。exact progressは必ずlive checkpoint unionを再取得して判断する。
