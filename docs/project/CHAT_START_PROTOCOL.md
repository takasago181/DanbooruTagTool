...
- Codex DEV:
  `AGENTS.md` の startup gate（`CURRENT_STATE.md` → current DEV Issueのlive取得 → `PERMANENT_RULES.md`）に従い、その後に現行Stage仕様・実装レポートを読む。
- fail-closed条件は `PERMANENT_RULES.md` の rule 11 に従う。
...
checkpointコメントはtask contractを黙って変更しない。目的・scope・禁止事項・完了条件はlive Issue本文を正本とし、最新コメントはcheckpoint・結果・証跡・履歴として確認する。
...
- DEVの現行Issue本文・state・完了条件を変更した場合は、live Issueを更新し、`CURRENT_STATE.md` のroutingと整合させる。
- Codexが守るべきDEVの目的・scope・禁止事項・完了条件はlive Issue本文から読む。
...
3. Codex DEVは `AGENTS.md` のstartup gateに従ってcurrent DEV Issueをlive取得する。常設班/TEMP/AUDITはそれぞれ明示された現行Issue/Gateを取得する。
...
DEV / Codex連携では、Codexが `CURRENT_STATE.md` のIssue番号とlive取得結果を一致確認する。
...
- `CURRENT_STATE.md` の Workstreams Registry はrouting/indexであり、live Issue本文、branch上の成果物、checkpointを置き換えるtask contractではない。
