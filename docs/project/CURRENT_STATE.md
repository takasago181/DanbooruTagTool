...
- #132の現行運用正本は `docs/issue132/parallel/CURRENT_AUTOMATION_OPERATION.md`。**300 identities / worker run は上限でノルマではない**。新規workは **25-row immutable checkpoint**、各25件の保存前二段階レビュー、4 checkpointごとの **100-row横断QA**。安全に確定した25/50/75...件は実行上限時にも保持し、raw throughputだけを品質PASSとしない。
