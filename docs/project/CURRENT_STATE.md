...
- Before the Issue #109 local integration gate, the local `catalog.db` still contains the historical **2,788 Special** catalog; it is rebuildable knowledge and must not be treated as the current production population. The one-shot integration target is the completed **3,059 Special** production profile above, while existing `UserData/user.db` remains user-owned state. The 29 deleted IDs stay absent as stable-ID gaps.
