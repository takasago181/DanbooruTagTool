...
**Issue #30 / Stage10 representative evaluator calibration design ready**

Issue #44の3-evaluator desk coverageは完了し、#30へのhandoffも同期済み。current core DEVは #30。
`CURRENT_DEV_TASK.md` は Source Issue #30 / `CALIBRATION_DESIGN_READY` に同期済み。Codexはlatest mainを取得し、Issue #30 / CURRENT_STATE / CURRENT_DEV_TASKの一致を確認してから作業継続する。
...
| `UIJA-PARENT:#34` | OPEN / CROSS-CUTTING / SEARCH_RELEVANCE_REMAINS | bilingual search relevance and remaining parent UI concerns | Issue #34 | `anal -> piano/analog...` 等のsubstring/fuzzy search noiseが主要残件。#35 UI presentationは完了、#36 translationは別lane | Issue #34 current body |
| `TEMP:#30` | ACTIVE / CORE_DEV / CALIBRATION_DESIGN_READY | Forge Neo evaluator calibration & A/B automation | `codex/issue30-calibration-design` | 32 cases × 4 images = planned 128 images のdesignはreviewable。controlled executionは未開始 | Issue #30 current body + `CURRENT_DEV_TASK.md` |
| `PROMPT:#5` | GATED | Stage10 formal Prompt handoff | Issue #5 | #30 calibration + #42 result後にformal handoff | Issue #5 |
| `PREP:#42` | RESERVED / GATED | Stage10 pre-evaluation product-purpose improvement | Issue #42 | dictionary freeze + #44 coverage satisfied。#30 calibration後、material UI-JA/data homework完了または明示分離後に実施 | Issue #42 |
...
### #30 Forge Neo evaluator calibration — CURRENT CORE DEV

- infrastructure: **PASS_PIPELINE**
- final dictionary freeze: **SATISFIED**
- #44 exact evaluator coverage: **SATISFIED**
- current phase: **CALIBRATION_DESIGN_READY / controlled execution not yet started**
...
- controlled 128-image calibration starts only after explicit authorization
...
#35 UI presentation work is completed; #36 V5 owns translation-data repair. Before #42 activation, the material search concern must be completed or explicitly separated from the Stage10 Gate.
...
Historical #46 is not a dependency.
...
12. #34 bilingual search relevance/noise — **REMAINS / resolve or explicitly separate before #42**
13. #30 calibration design — **DESIGN_READY / CURRENT CORE DEV**
14. #30 controlled representative calibration — **NOT STARTED / explicit authorization required**
15. #42 product-purpose improvement pass — REMAINS / after #30 calibration and material UI-JA/data homework complete or separated
16. #5 formal Prompt handoff — REMAINS / after #30 + #42
...
Parallel now:
- Core DEV: `#30 design-ready -> explicit authorization -> controlled representative calibration`
...
- Maintenance/cross-cutting: `#24 protected-data safety`, `#34 bilingual search relevance`
...
1. #30 completed calibration designをreview/acceptし、explicit authorization後のみcontrolled 128-image calibrationへ進む
...
4. #34 bilingual search relevance/noiseをresolveまたはStage10 Gateから明示分離
...
- Issue #30 state = **CALIBRATION_DESIGN_READY / CONTROLLED_EXECUTION_NOT_YET_STARTED**
- Issue #30 / CURRENT_STATE / CURRENT_DEV_TASK が一致しない場合、Codexは作業継続しない
