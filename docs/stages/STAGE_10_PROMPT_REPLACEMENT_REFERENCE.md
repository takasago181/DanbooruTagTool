...
- 現在Stageは `docs/project/CURRENT_STATE.md` を正本として参照する。この文書自体はStage10本番開始を意味しない。
...
この運用の目的は、ユーザーに毎回タグ探索・Prompt再構築・大量の手動差し替えをさせることではない。

- 手動差し替えは**標準経路にしない**。
- PROMPT班は、対応可能な部分をすべて完成Promptとして組み、ユーザーがそのまま利用・比較できる状態へ最大限近づける。
- 具体記述できない箇所がある場合も、Prompt全体を曖昧化せず、**本当に必要な最小箇所だけ**を局所的な差し替え札にする。
- 差し替え札を理由にcamera / pose / visibility / quality / model-family構造 / negative等の設計までユーザーへ丸投げしない。
- 1つの概念について複数箇所へ重複した札を作らず、可能な限り1スロットへ集約する。
- 差し替えが必要でも、Special Core Dictionary（現行snapshotの互換識別子は `Special2788`）からPROMPT班が第一候補を選び、原則3候補程度を日本語付きで提示する。ユーザー自身に2788件を検索させない。
- 直接Promptへ含められる候補はPROMPT班側で反映する。含められない場合のみ、最小スロットとして残す。
- 「できない可能性がある」だけで予防的に大量の置換札へ逃がさない。実際の境界に必要な箇所だけを対象にする。

### スロット最小化の明示的品質目標

本プロジェクトのツール作成・検証に用いる一次的なPrompt支援では、**残る差し替えスロットの数と範囲を、実務上可能な限り減らす**ことをPROMPT班の明示的な品質目標とする。

- 常にゼロスロットを保証するものではない。
- ただし、対応可能な内容まで予防的にスロット化して数を増やさない。
- 同一の核心概念を複数スロットへ分散させず、意味を壊さない範囲で可能な限り集約する。
- **スロット数を減らすために、ユーザー指定の成人Special・性癖の強度、具体性、identityを勝手に弱めない。**
- 対応可能な範囲では、一般語への希釈、ソフト化、婉曲化、曖昧化によって見かけ上スロットを減らすことを禁止する。
- 境界が残る場合は、その境界だけを局所スロット化し、camera / pose / visibility / geometry / quality / model-family構造 / negative / support等、記述可能な周辺部分は元の強度を保ったまま最大限完成させる。
- ユーザーの作業は、必要な場合でも原則として「候補を選ぶ／最小スロットを置き換える」程度に抑える。

目標は、差し替えが必要なケースでも**ユーザー作業を『候補を選ぶ／最小スロットを置き換える』程度に抑え、Prompt設計そのものはPROMPT班が担う**こと。
...
4. 置換が必要な場合、Special Core Dictionary参照辞書を使い、実在候補を併記する。
...
11. 複数の置換箇所がある場合は、A / B / Cのように部位・行為・状態を分ける。ただし同一概念の無用な分割は避ける。
12. Stage10の成人向けPromptでは未成年対象語を使用しない。
13. 成人・合意は重要な前提だが、それだけを根拠に「すべて無制限に直接記述可能」と約束しない。一方で、その境界を理由に制作目的から外れるほど多数の手動差し替えを要求しない。
14. スロットが必要な場合でも、その数・範囲を実務上可能な限り減らす。減らすために成人Specialの意味・強度・具体性を薄めることはしない。
...
この候補表はPrompt本文の代用品ではない。本文は可能な限り完成状態で先に作り、候補表は最小差し替え箇所の選択支援として添える。
...
元の `Special2788` snapshot dataを置き換える正本ではない。Special Core Dictionaryのタグ探索・差し替え候補提示のための補助資料としてのみ使う。
