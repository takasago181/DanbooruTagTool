...
Read only this card, coordinator_status.json, three lane status files, repair_status.json when present, plus latest Issue132 CI conclusion. Fetch CI job logs only when run_id/head_sha changed or the failure class changed.
...
- summarize repair debt trend from repair_status.json;
