...
3. **Evidence Ledger** — represents `EXTERNAL_AUTHORITY`, `APPROVED_REPO_EVIDENCE`, `REVIEWED_QUALIFIER_COPYRIGHT`, `ROOT_POLICY_NORMALIZATION`, `REVIEWED_VARIANT_AUTHORITY`, and the constrained Tier-B `SAFE_STRUCTURAL_VARIANT` basis with deterministic evidence IDs and provenance. A safe exact terminal Copyright-family qualifier can establish membership even when inner qualifiers are present, provided no prior qualifier signals a different reviewed HOME or blocked broad/company/platform/event/crossover class. Structural HOME inheritance requires one exact existing base Character with a single validated HOME; when an outer Copyright qualifier exists it must resolve to that same HOME. A unique single-modifier catalog form may inherit from its exact base without proving costume/event officiality, while known collision/crossover and blocked broad/non-home qualifiers remain excluded. Decision CSVs alone are never evidence.
...
Each pipeline run also writes `reports/RESEARCH_UNIT_CLOSURE_SWEEP_V3.csv`, classifying every exact-fingerprint OPEN unit against validated direct/family/variant paths, approved evidence, candidate structure, origin handoff, and candidate roots. Candidate-only structure remains a research lead, never HOME authority. Its `work_bucket` column separately prioritizes `SAFE_STRUCTURE_READY`, `FAMILY_ROSTER_HIGH_YIELD`, `VARIANT_BASE_READY`, `DIRECT_AUTHORITY_RESEARCH`, `AMBIGUOUS_LOW_YIELD`, `IDENTITY_BLOCKED`, `POLICY_BLOCKED`, `STRUCTURAL_NO_SAFE_PATH`, and `CONFLICT_REVIEW`; these are workflow buckets, not evidence or terminal decisions. `STRUCTURAL_NO_SAFE_PATH` can only be terminalized when the exact current member fingerprint has no admissible or candidate path; the validator requires one classification row per OPEN unit. A HOME conflict in this census is counted only when its direct/family/variant path is supported by validated Evidence Ledger relations; candidate-only `MEMBER_OF` or `VARIANT_OF` edges cannot manufacture a conflict.
