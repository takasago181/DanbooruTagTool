...
Do not satisfy mandatory lanes by blanket-defer boilerplate. The readiness gate enforces a deliberately small productivity floor: every non-exempt mandatory discovery group must produce at least one grounded HOME_CONFIRMED member, mandatory family review work must produce at least one autonomous FAMILY_QUALIFIER PASS overall, and mandatory ready-variant work must produce at least one autonomous VARIANT_CHARACTER PASS overall. `original` and `indie_virtual_youtuber` are the only zero-yield discovery-group exemptions. These are floors, not targets: continue adding all safe roster/family/variant authority you can prove. Revisit existing UNRESOLVED rows such as clean title abbreviations/normalizations when the cited source actually supports a stable canonical Copyright HOME.
