...
Terminal `UNRESOLVED` / `NEEDS_HIGHER_REASONING` rows are review conclusions, not shortcuts. Every terminal review requires grounded evidence plus substantive notes. This applies to family, Character, discovery-group and variant-pattern reviews. Decision shards and the compatibility decision ledger are never valid `REPO:` evidence for their own decisions.
